...
class BDIService:
    ...
    def from_config(cls, bdi_model: dict, event_bus: EventBus, resource_manager: ResourceManager,
        ...
        return cls(bdi_model, config.get("topic_intention"), config.get("topic_desire"),
                   event_bus, resource_manager)

    def __init__(self, bdi_model: dict, intention_topic: str, desire_topic: str,
    ...
    def _process(self, event: Event):
        ...
        try:
            if event.metadata.topic == self._intention_topic:
                ...
                intentions_event = Event.for_payload(IntentionEvent(intentions_payload), source=event)
                self._event_bus.publish(self._intention_topic, intentions_event)
